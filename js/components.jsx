/* =====================================================================
   components.jsx — presentational sections for the CV
   Exposes section components on window for app.jsx to consume.
   ===================================================================== */
...
const Icon = {
  ...
  pin: (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...p}><path d="M12 21s7-5.4 7-11a7 7 0 1 0-14 0c0 5.6 7 11 7 11Z"/><circle cx="12" cy="10" r="2.6"/></svg>),
...
/* ---------------- section wrapper ---------------- */
function Section({ id, num, title, sub, children, label }) {
  return (
    ...
    <Section id="experience" num="02" title={T.sections.experience} label={T.sections.experience}>
      <div className="timeline">
        {T.experience.map((x, i) => (
          <div className={"tl-item reveal" + (x.current ? " cur" : "")} key={i} style={{ animationDelay: (i * 70) + "ms" }}>
  ...
  return (
    <Section id="projects" num="04" title={T.sections.projects} label={T.sections.projects}>
      <div className="proj-grid">
        {T.projects.map((p, i) => (
          <div className="proj-card reveal" key={i} style={{ animationDelay: (i * 80) + "ms" }}>
...
/* ---------------- EDUCATION + SKILLS ---------------- */
function EduSkills({ T, lang }) {
  const soft = window.CV.soft[lang];
  const e = T.education;
  return (
...
/* ---------------- GITHUB ---------------- */
function GitHub({ T, meta }) {
  // deterministic illustrative heatmap
    ...
    <Section id="github" num="06" title={T.sections.github} label={T.sections.github}>
      <div className="gh-card reveal">
        ...
        <div className="heatmap">
          {cells.map((l, i) => (
            <div className="heat-cell" key={i} style={{ background: lvlColor(l) }}></div>
          ))}
        </div>
        <div className="gh-legend">
          less
          {[0, 1, 2, 3, 4].map((l) => <span className="sq" key={l} style={{ background: lvlColor(l) }}></span>)}
          more
        </div>
...
Object.assign(window, {
  Icon, Section, Hero, About, Experience, Stack, Projects, EduSkills, GitHub, Contact,
});
