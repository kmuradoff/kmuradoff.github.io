/* =====================================================================
   ...
   Vanilla Three.js. Exposes window.initTechSphere(container, opts).
   Drag to spin · auto-rotates · hovered tags brighten.
   ===================================================================== */
window.initTechSphere = function (container, opts) {
  opts = opts || {};
  ...
  const words = [];
  window.CV.stack.forEach((c) => c.items.forEach((w) => words.push(w)));
  ...
  function setTheme() {
    // rebuild tag textures to match theme text color
    tags.forEach((sp) => {
  ...
  function frame() {
    ...
    // depth-based opacity (front tags brighter)
    tags.forEach((sp) => {
      const wp = sp.getWorldPosition(new THREE.Vector3());
      const z = wp.z; // -R..R
      const t = (z + R) / (2 * R); // 0 back .. 1 front
