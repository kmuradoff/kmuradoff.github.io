  ...
  ctx = gsap.context(() => {
    ...
    if (atTop) {
      gsap.timeline({ defaults: { ease: 'power3.out' } })
        .fromTo('.hero-deco',      { opacity: 0 },                          { opacity: 1, duration: 2.2, ease: 'power2.out' }, 0)
        .fromTo('.hero-name',      { y: 72, opacity: 0, skewX: 4, filter: 'blur(8px)' },
                                   { y: 0,  opacity: 1, skewX: 0, filter: 'blur(0px)', duration: 1.1, delay: 0.1 }, 0)
      ...
      gsap.set(['.hero-deco','.hero-name','.hero-role','.hero-tag','.hero-cta .chip','.scroll-cue'],
